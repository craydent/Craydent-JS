...
Craydent = {
    browser:{
        CURRENT:(_ie != -1 && 'Internet Explorer') || (_chrm != -1 && 'Chrome') || (_ff != -1 && 'Firefox') || (_saf != -1 && 'Safari'),
    ...
    engine:{
        CURRENT:(_amay && 'Amaya') || (_gekk && 'Gekko') || (_khtm && 'KHTML') || (_pres && 'Presto') || (_prin && 'Prince') || (_trid && 'Trident') || (_webk && 'WebKit'),
        AMAYA:_amay,
        GEKKO:_gekk,
        KHTML:_khtm,
        PRESTO:_pres, 
    ...
    os:{
        CURRENT:(_droid && 'Android') || (_bbery && 'BlackBerry') || (_linx && 'Linux') || ((_ipad || _ifon || _ipod) && 'iOS') || (_mac && 'Mac') || (_palm && 'PalmOS') || (_symb && 'Symbian') || (_win && 'Windows') || (_winm && 'Windows Mobile'),
        ANDROID:_droid,
        BLACKBERRY:_bbery,
        LINUX:_linx,
    ...
    device:{
        CURRENT:(_droid && 'Android') || (_bbery && 'BlackBerry') || (_ipad && 'iPad') || (_ifon && 'iPhone') || (_ipod && 'iPod') || (_linx && 'Linux') || (_mac && 'Mac') || (_palm && 'PalmOS') || (_symb && 'Symbian') || (_win && 'Windows') || (_winm && 'Windows Mobile'),
        ANDROID:_droid,
        BLACKBERRY:_bbery,
        IPAD:_ipad,
        IPHONE:_ifon,
...
function _craydentSelector (by, overwrite, object, single) {
    try {
        ...
        var elem = $d[by][object] || ($d[by][object] = ($d[by](object)||$w[overwrite](object)));
//        if (single) {
//            return ((elem && ((elem.length && elem[0]) || elem)) || $w[overwrite](object)[0]);
//        }
//        return (elem || $w[overwrite](object));
        if (single) {
            return elem && ((elem.length && elem[0]) || elem);
        }
        return (elem);
    } catch (e) {
        error('selector', e);
...
$c.hide = function (object) {
    _displayHelper(object, "hide");
};
$c.toggle = function (object) {
...
//      }
        ...
        extra_code = (name in {"height":1,"width":1}) ? "" : "if(!obj && this == $w){return;}",
    ...
    **/  
        ...
        if (!dt.getDate()) {
            ...
            for (var i = 0, len = chars.length; i < len; i++) {
                ...
                if (!dt && (parts = dtstring.match(/(\d{1,2})\/(\d{1,2})-(\d{2,4})(.*)/)) && parts.length > 1) {
                    dt = new Date(parts[2] + "/" + parts[1] + "/" + parts[3] + parts[4]);
                } else if (!dt && (parts = dtstring.match(/(\d{1,2})-([a-zA-Z]{3,9})-(\d{2,4})(.*)/)) && parts.length > 1) {
...
_extend(Array, 'condense', function (check_values) {
    try {
        for (var i = 0, len = this.length; i < len; i++) {
            if (check_values) {
                var index = 0;
...
function _sortHelper (a,b,p,props,lookup,useLookup,lookupfunc,lprimer) {
    useLookup = useLookup || false;
    p = p || 0;
    var prop = props[p];
    ...
    aVal = useLookup ? ((lookup && lookup[a][prop]) || a[prop]) : craftVal(a,prop),
    bVal = useLookup ? ((lookup && lookup[b][prop]) || b[prop]) : craftVal(b,prop);
...
_extend(Array, 'sortBy', function(props, rev, primer, lookup){
    ...
    prop_sort = function (a,b,p) {
        ...
        var aVal = (lookup && lookup[a][prop]) || a[prop],
        bVal = (lookup && lookup[b][prop]) || b[prop];
...
//    return this.sort(prop_sort);
    return this.sort(function(a,b,p){ 
        return _sortHelper(a, b, p,props,lookup, true);
    });
}, true);
...
//    return this.sort(prop_sort);
    return this.sort(function(a,b,p){ 
        return _sortHelper(a, b, p, props, lookup, false, lookupfunc,lprimer);
    });
},true);
...
_ao("merge", function (secondary, condition) {//shareOnly) {
    try {
        var objtmp = $c.copyObject(this),
        recurse = $c.isObject(condition) || $c.isArray(condition);

        for (prop in secondary){
            if (secondary.hasOwnProperty(prop)) {
                if ($c.isBoolean(condition) && condition) {
                    // passing share Only
                    if (objtmp[prop]) {
                        objtmp[prop] = secondary[prop];
                    }
                } else if ($c.isFunction(condition)) {
                    if ($c.isArray(objtmp) && objtmp[prop] && condition(objtmp[prop], secondary[prop])) {
                        objtmp[prop] = secondary[prop];
                        continue;
                    }
...
function _getDimension(isBody, dimension) {
    try {
        var dimProp = dimension.capitalize();
        if (this == $w) {
            var de = $d.documentElement;
            function isClientMeasurementsOff () { 
            ...
            if (typeof $d['client'+dimProp] == "number") {
                return $d['client'+dimProp];
            }
            if (de && de['client'+dimProp]===0 || isClientMeasurementsOff()) {
                 return $d.body['client'+dimProp];
            }
            return de['client'+dimProp];
        }
        ...
        return (this.getClientRects && this.getClientRects()[0][dimension]) || this["offset" + dimProp] || this["scroll" + dimProp];
    } catch (e) {
...
function _runFuncArray(funcs, args) {
...
//        params.onsuccess = ($c.isArray(params.onsuccess) ? params.onsuccess : [params.onsuccess]) || [function (data,hitch,thiss,context) {}];
        ...
        if (params.dataType.toLowerCase() == 'jsonp') {
            ...
            insert = 'insertBefore';
            params.tag = $d.createElement('script');
            while ($w[func]) {
...
//                    (!data.hasErrors && 
//                        (params.onsuccess.call((params.context||params.tag),data, params.hitch, params.thiss, params.context) || true)) || 
//                        params.onerror.call((params.context||params.tag),data, params.hitch, params.thiss, params.context);
                    
                    (!data.hasErrors && 
                        (_runFuncArray.call((params.context||params.tag),params.onsuccess, [data, params.hitch, params.thiss, params.context]) || true)) || 
                        _runFuncArray.call((params.context||params.tag),params.onerror,[data, params.hitch, params.thiss, params.context]);
                    
                    _runFuncArray(params.oncomplete);
//                    params.oncomplete();
                }
            }
            ...
            url += (params.url.indexOf('?') != -1 ? "&" : "?") + params.jsonp + "=" + func + (query || "");         
            
            params.tag['type'] = "text/javascript";
            params.tag.async = "async";
            params.tag['src'] = url;

            // Attach handlers for all browsers
            params.tag.onload = params.tag.onreadystatechange = function(ev) {
                try {
            ...
            _runFuncArray(params.onbefore, [params.tag, this]);
//            params.onbefore(params.tag, this);
            head[insert](params.tag, head.firstChild);
            rtn = params.tag;
        } else {
            ...
            _runFuncArray(params.onbefore, [httpRequest, this]);
//            params.onbefore(httpRequest, this);
            httpRequest.onreadystatechange = function (xp) {
                ...
                if (data) {
                    _runFuncArray.call((params.context||this),params.onsuccess, [data, params.hitch, params.thiss, params.context, this.status]);
                } else if (this.readyState == 4) {
                    try {
                        _runFuncArray.call((params.context||this),params.onerror, [eval(this.responseText), params.hitch, params.thiss, params.context, this.status]);
                    } catch (e) {
                        _runFuncArray.call((params.context||this),params.onerror, [this.responseText, params.hitch, params.thiss, params.context, this.status]);
                    }
                }
                _runFuncArray(params.oncomplete);
            };
...
function fillTemplate (htmlTemplate, objs) {
    try {
        ...
        for (var i = 0, len = objs.length; i < len; i++) {
            ...
            $c.isObject(objs[i]) && $c.duplicate(obj, objs[i], true) || (obj = objs[i]);
            for (var j = 0, jlen = $c.TEMPLATE_VARS.length; j < jlen; j++) {
                ...
                template = htmlTemplate.ireplace_all("${"+variable+"}", value);
            }
            template = htmlTemplate.ireplace_all("${this}",
                ($c.isObject(obj) && parseRaw(obj,true).replace_all(';', ';\\').replace_all('[','\\[').replace(']','\\]') || obj)).ireplace_all("${index}", i);
            while ((match=/\$\{this\.(.+?)\}/.exec(template))) {
                variable = parseRaw($c.propertyValue(obj, match[1]), true);
                template= template.ireplace_all("${this."+match[1]+"}", variable);
            }
            for (property in obj) {
                ...
                if (obj.hasOwnProperty(property)) {
                    if (template != "" && obj[property] != undefined && obj[property].toString().indexOf('\n') != -1) {
                        obj[property] = obj[property].replace_all('\n', '<br />');
                    }
                    if (template != "" && obj[property] != undefined) {
                        template = template.ireplace_all("${" + property + "}", 
                            parseRaw(obj[property], true).replace_all(';', ';\\').replace_all('[','\\[').replace_all(']','\\]'))
                        .ireplace_all('${dataproperties}', "data-" + property + "='" + 
            ...
            // special run sytax
            function __run_replace (reg, template, use_run) {
                try {
...
//                    use_run && (pre="RUN\\[",post="\\]", split_param=/;(?!\\)/);
                    ...
                    while (reg.test(template)) {
                        ...
                        if (match[1]) {
                            ...
                            funcValue = match[1].replace(/\\\[/gi,'[').replace(/\\\]/gi,']').split(split_param);
...
//                            template = template.ireplace_all("${" + pre + __convert_regex_safe(match[1]) + post +"}", $w[func] ? $w[func].apply(objs, funcValue) : "");
                            template = template.ireplace_all("${" + pre + match[1] + post +"}", $w[func] ? $w[func].apply(objs, funcValue) : "");
                        }
                    }
                    return template;
                } catch (e) {
            ...
            template = __run_replace (/\$\{RUN\[(.+?)\]\}/, template, true);
            template = template.replace(/(\$\{[a-zA-Z$_]*\})/gi, '');
            var tmp, rptmp;
            tmp = /\$\{(.+?\|\|?.+?)\}/.exec(template);
            if (tmp && tmp[1]) {
                tmp = tmp[1];
                rptmp = (tmp && "__or|"+tmp.ireplace_all('||', "|") || "");
                template = template.ireplace_all(tmp, rptmp);
            }
            template = __run_replace (/\$\{(.+?\|?.+?)\}/, template, false);
                        
            html += template.ireplace_all('${dataproperties}', "").replace(/\$\{.*?\}/gi,"").replace_all(';\\', ';');
        }
        return html;
    } catch (e) {
