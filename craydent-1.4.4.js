    ...
    try {
        ...
        var loop_func = function (prop, original) {
            if (original.hasOwnProperty(prop) && original[prop] && (!original[prop].isFunction() || !recursive)) {
                ...
                if (index != -1) {
                    arguments[argIndex+2].command += new_path + "="+ref.objects[index].path+";";
                    return;
                }
                ...
                if (typeof(original[prop]) == "object" && recursive) {
                    ...
                    $w.duplicate(this[prop], original[prop], true, ref, new_path, arguments[argIndex+2]);
                    return;
                }
...
//        var objtmp = {};
        ...
        recurse = isObject(condition) || isArray(condition);
...
_ao("isObject", function () {
    try {
        ...
        return (this.constructor == Object || this instanceof Object);
    } catch (e) {
        error('isObject', e);
    }
});
    ...
    try {
        ...
        if (params.dataType.toLowerCase() == 'jsonp') {
            ...
            $w[func] = function (data) {
                (!data.hasErrors && 
                    (params.onsuccess(data, '', params.thiss, params.context) || true)) || 
                    params.onerror(data, '', params.thiss, params.context);
            }
            ...
            httpRequest.onreadystatechange = function (xp) {
                ...
                if (data) {
                    params.onsuccess(data, this.status, this, params.context);
...
function fillTemplate (htmlTemplate, objs) {
    try {
        ...
        for (var i = 0, len = objs.length; i < len; i++) {
            ...
            $w.isObject(objs[i]) && duplicate(obj, objs[i], true) || (obj = objs[i]);
