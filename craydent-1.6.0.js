...
//            template = htmlTemplate.replace(new RegExp("\\$\\{(.*)?\\|\\|(.*)?\\}", "gi"),"$${RUN[__or;$${$1};$${$2}]}");
...
//                    regex = new RegExp("\\$\\{" + match[1].replace('\n','')
//                        .replace(/\\/gi,"\\\\")
//                        .replace(/\$/gi, "\\$")
//                        .replace(/\//gi, "\\/")
//                        .replace(/\^/gi, "\\^")
...
//                    template = template.replace(match[1], (match[1] = match[1].replace(/\\\[/gi, '[').replace(/\\\]/gi, ']')));
//                    regex = new RegExp("\\$\\{" + pre + match[1].replace('\n','')
//                        .replace(/\\/gi,"\\\\")
//                        .replace(/\$/gi, "\\$")
//                        .replace(/\//gi, "\\/")
//                        .replace(/\^/gi, "\\^")
...
//            }
                ...
                try {
                    ...
                    while (reg.test(template)) {
                        ...
                        if (match[1]) {
                            ...
                            //                    match[1] = match[1].replace(/\\\[/gi,'[').replace(/\\\]/gi,']');
                            template = template.replace(match[1], (match[1] = match[1].replace(/\\\[/gi, '[').replace(/\\\]/gi, ']')));
                            regex = new RegExp("\\$\\{" + pre + __convert_regex_safe(match[1]) + post +"\\}", "gi");
                            template = template.replace(regex, $w[func] ? $w[func].apply(objs, funcValue) : "");
                        }
                    }
                    return template;
            ...
            if (tmp && tmp[1]) {
                tmp = tmp[1];
                rptmp = (tmp && "__or|"+tmp.replace(/\|\|/gi, "|") || "");
                template = template.replace(RegExp(__convert_regex_safe(tmp),"gi"), rptmp);
            }
