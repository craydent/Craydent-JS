...
function _defineFunction (name, func, override) {
    if ($w.hasOwnProperty(name) && !override) {return;}
...
Date.prototype.format = Date.prototype.format || function (format) {
    ...
    timezones = {
        ...
        'Heure AvancÃ©e d\'Europe Centrale francised name for CEST':'HAEC',
        'Hong Kong Time':'HKT',
...
_ao("duplicate", function (original, recursive/*, ref, current_path, exec*/) {
    try {
        ...
        var ref = arguments[2] || {objects:[{obj:original,path:"this"}]},
        current_path = arguments[3] || "this";
        (arguments[4] || (arguments[4] = {})) && (arguments[4].command = arguments[4].command || "");
        ...
        var loop_func = function (prop, original) {
            if (original.hasOwnProperty(prop) && original[prop] && (!original[prop].isFunction() || !recursive)) {
                ...
                if (index != -1) {
                    arguments[4].command += new_path + "="+ref.objects[index].path+";";
                    return;
                }

                if (typeof(original[prop]) == "object" && recursive) {
                    this[prop] = original[prop].constructor();
                    ref.objects.push({obj:original[prop],path:new_path});
                    this[prop].duplicate(original[prop], true, ref, new_path, arguments[4]);
                    return;
                }
        ...
        if (isArray(original)) {
            for (var i = 0, len = original.length; i < len; i++){
                loop_func.call(this, i, original, ref, current_path, arguments[4]);
            }
        } else {
            for (prop in original){
                loop_func.call(this, prop, original, ref, current_path, arguments[4]);
            }
        }
        
        if (!arguments[3]) {
            eval(arguments[4].command);
        }
...
_ao("merge", function (secondary, shareOnly) {
    try {
        var obj = {};
        obj.duplicate(this);
...
function fillTemplate (htmlTemplate, objs) {
    try {
        ...
        for (var i = 0, len = objs.length; i < len; i++) {
            ...
            objs[i].isObject() && obj.duplicate(objs[i], true) || (obj = objs[i]);
//            obj.duplicate(objs[i], true);
            for (var j = 0, jlen = Craydent.TEMPLATE_VARS.length; j < jlen; j++) {
                ...
                value = isFunction(value) ? value(obj,i):value;
...
//            template = htmlTemplate.replace(new RegExp("\\$\\{this\\}", "gi"),parseRaw(obj)).replace(new RegExp("\\$\\{index}", "gi"), i);
            template = htmlTemplate.replace(new RegExp("\\$\\{this\\}", "gi"),
                (obj.isObject() && parseRaw(obj,true).replace(/;/g, ';\\').replace(/\[/gi,'\\[').replace(/\]/gi,'\\]') || obj)).replace(new RegExp("\\$\\{index}", "gi"), i);
            while ((match=/\$\{this\.(.+?)\}/.exec(template))) {
                //variable = propertyValue(obj, match[1]);
                variable = parseRaw(propertyValue(obj, match[1]));
...
function zipit(files, content/*=NULL*/) {
    files = (content && files.isString() && [{name:files,content:content}]) || files.isObject() && [files] || files.isArray() && files;
    var zip = new JSZip();
