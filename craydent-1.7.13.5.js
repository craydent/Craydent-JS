...
/---------------------------------------------------------------------------------------------------------------*/
_extend(Date, 'format', function (format) {
...
function $GET(variable, options) {//used to retrieve variables in the url
    try {
        ...
        } else if (options.url || ($c.isString(options) && (options.indexOf("?") != -1 || options.indexOf("#") != -1))) {
...
function error(fname, e) {
    try {
        if ($GET("debug") == "true" || $c.DEBUG_MODE) {
            cout("Error in " + fname + "\n" + (e.description || e));
        }
    } catch (e) {
        cout("Error in " + fname + "\n" + (e.description || e));
