    ...
    try {
        ...
        for (prop in secondary){
            if (secondary.hasOwnProperty(prop)) {
                objtmp[prop] = condition (objtmp, secondary[prop]);	
            }
        }
        return objtmp;
    } catch (e) {
...
function fillTemplate (htmlTemplate, objs) {
    try {
        ...
        for (var i = 0, len = objs.length; i < len; i++) {
            ...
            isObject(objs[i]) && duplicate(obj, objs[i], true) || (obj = objs[i]);
            ...
            template = htmlTemplate.replace(new RegExp("\\$\\{this\\}", "gi"),
                (isObject(obj) && parseRaw(obj,true).replace(/;/g, ';\\').replace(/\[/gi,'\\[').replace(/\]/gi,'\\]') || obj)).replace(new RegExp("\\$\\{index}", "gi"), i);
            while ((match=/\$\{this\.(.+?)\}/.exec(template))) {
                //variable = propertyValue(obj, match[1]);
                variable = parseRaw(propertyValue(obj, match[1]), true);
